...
function createPlatform(x, y) {
  ...
  if (Math.random() < 0.2) {
    let r = Math.random();
    let type;
    // 5% of the 20% chance becomes white (overall ≈1% chance)
    if (r < 0.05) {
      type = "white";
    } else {
      // For higher scores, thresholds change; here we scale remaining probability by 0.95.
      if (globalHighScore >= 6666) {
        if (r < 0.05 + 0.125 * 0.95) type = "jump";
        else if (r < 0.05 + 0.125 * 0.95 * 2) type = "lava";
        else if (r < 0.05 + 0.125 * 0.95 * 3) type = "red"; // will render as black
        ...
        else if (r < 0.375) type = "red"; // red will be drawn as black
  ...
  platforms.forEach(platform => {
    ...
    if (platform.powerup && !platform.powerup.collected) {
      ...
      switch(platform.powerup.type) {
        ...
        case "red": col = "black"; break; // red powerup now drawn in black
...
function gameLoop() {
  ...
  // Wrap drawing with white powerup effect if active.
